...
export enum CEP47Events {
  ...
  grant_gatekeeper = "grant_gatekeeper",
  init = "init",
  bid = 'bid',
...
export class CEP47Client {
  ...
  deploy: DeployUtil.ExecutableDeployItem;

  constructor(public nodeAddress: string, public networkName: string) {
    this.casperClient = new CasperClient(nodeAddress);
    this.contractClient = new Contract(this.casperClient);
    this.deploy = new DeployUtil.ExecutableDeployItem;
  }
  ...
  public setArgs(name:string,value:CLValue){
    this.deploy.setArg(name,value);
  }
  ...
  public async reserve_price() {
    return this.contractClient.queryContractData(['reserve_price']);
  }
  public async get_start() {
    return this.contractClient.queryContractData(['start_time']);
  }
  public async name() {
...
public async mintkyc(
  ...
  const runtimeArgs = RuntimeArgs.fromMap({
    recipient: CLValueBuilder.key(recipient),
    token_ids: CLOption(ids.map(id => CLValueBuilder.u256(id))),
    token_metas: CLValueBuilder.list(metas.map(meta => toCLMap(meta)))
  });
  ...
  public async grant_gatekeeper(
    gatekeeper: CLKeyParameters,
    //ids: string[],
    paymentAmount: string,
    deploySender: CLPublicKey,
    keys?: Keys.AsymmetricKey[]
    ...
    const runtimeArgs = RuntimeArgs.fromMap({
      gatekeeper: CLValueBuilder.key(gatekeeper)
      //token_ids: CLValueBuilder.list(ids.map(id => CLValueBuilder.u256(id))),
     // token_metas: CLValueBuilder.list(metas.map(meta => toCLMap(meta)))
    });
  ...
  public async init(
    AUCTION_PACKAGE_HASH: CLKeyParameters,
    AUCTION_CONTRACT_HASH: CLKeyParameters,
    //ids: string[],
    paymentAmount: string,
    deploySender: CLPublicKey,
    keys?: Keys.AsymmetricKey[]
    ...
    const runtimeArgs = RuntimeArgs.fromMap({
      AUCTION_PACKAGE_HASH: CLValueBuilder.key(AUCTION_PACKAGE_HASH.slice(5)),
      AUCTION_CONTRACT_HASH:CLValueBuilder.key(AUCTION_CONTRACT_HASH.slice(5))
      //token_ids: CLValueBuilder.list(ids.map(id => CLValueBuilder.u256(id))),
     // token_metas: CLValueBuilder.list(metas.map(meta => toCLMap(meta)))
    });
  ...
  public async call(
    ...
    const runtimeArgs = RuntimeArgs.fromMap({
      ...
      auction_contract:CLValueBuilder.key(auction_contract.slice(5)),
      purse_name:CLValueBuilder.string(purse_name)
      //token_ids: CLValueBuilder.list(ids.map(id => CLValueBuilder.u256(id))),
     // token_metas: CLValueBuilder.list(metas.map(meta => toCLMap(meta)))
    });
  ...
  public async bid(
    ...
    const runtimeArgs = RuntimeArgs.fromMap({
      bid: CLValueBuilder.u512(bid),
      bidpurse:CLURef.fromFormattedStr(bidpurse)
      //token_ids: CLValueBuilder.list(ids.map(id => CLValueBuilder.u256(id))),
     // token_metas: CLValueBuilder.list(metas.map(meta => toCLMap(meta)))
    });
